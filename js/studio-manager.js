(function () {
  ...
  if (resetPasscode) {
    resetPasscode.addEventListener("click", function () {
      // A generic confirm() dialog is a single click most people click
      // through without reading — it was not a meaningful barrier
      // against someone who simply found this URL. Requiring an exact
      // typed phrase means resetting is a deliberate act, not an
      // accidental or casual one. This is still NOT real security (see
      // the disclosure below and PROJECT_RULES.md → "Private Management
      // System") — it only raises the bar against casual/opportunistic
      // access, since anyone with browser devtools can already read or
      // clear localStorage/IndexedDB directly regardless of this gate.
      var typed = window.prompt(
        'Resetting does NOT delete any locally saved draft projects or media — ' +
        "whoever sets the next passcode will see them. If you're sure, type RESET (all caps) to continue."
      );
