 ...
 * footer already renders); nothing about Discord/Instagram/etc. is
 * duplicated here.
 * =============================================================================
 */
...
(function () {
  ...
  if (Array.isArray(cfg.social)) {
    cfg.social.forEach(function (item) {
      ...
      methods.push({
        name: item.name,
        value: item.href && item.href !== "#" ? item.href.replace(/^https?:\/\//, "") : "",
        href: item.href || "#",
        icon: item.icon,
        external: true,
      });
  ...
  if (!methods.length) {
    if (emptyState) {
      ...
      emptyState.textContent = contactCfg.emptyStateText || "No contact methods listed yet.";
    }
    return;
  }
