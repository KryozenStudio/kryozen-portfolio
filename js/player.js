...
(function () {
  ...
  function openPlayer(project, originEl) {
    ...
    if (!wasAlreadyOpen) {
      ...
      // showModal() just auto-focused the dialog's first focusable
      // descendant (the close button) using the browser's default
      // scroll-into-view behavior. The dialog is fixed-position and
      // already fully on screen, so that scroll is never actually
      // needed — but on mobile it still runs, visible as a small extra
      // jump the instant the player opens. Re-focusing the same element
      // with preventScroll cancels that scroll without giving up focus.
