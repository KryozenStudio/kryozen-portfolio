...
document.getElementById("google-login").addEventListener("click",async function(){message("Opening Google authentication…");var r=await sb.auth.signInWithOAuth({provider:"google",options:{redirectTo:window.location.origin+"/admin.html"}});if(r.error)message(r.error.message);});
