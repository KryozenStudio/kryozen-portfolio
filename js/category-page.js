...
function render(){if(!grid)return;var q=(search?search.value:"").trim().toLowerCase();var list=projects.filter(function(p){var hay=((p.title||"")+" "+(p.description||"")+" "+(p.software||[]).join(" ")).toLowerCase();var matchQ=!q||hay.indexOf(q)!==-1;var matchF=activeFilter==="All"||(p.software||[]).indexOf(activeFilter)!==-1;return matchQ&&matchF});grid.innerHTML="";if(count)count.textContent=list.length+" project"+(list.length===1?"":"s");if(!list.length){var e=document.createElement("div");e.className="category-page__empty";e.textContent="No projects match this search.";grid.appendChild(e);return}list.forEach(function(p){grid.appendChild(card(p))})}
