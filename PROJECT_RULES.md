...
```
kryozen-studio/
├── index.html              → the public site (single page, one HTML file per page pattern)
├── admin/
│   ├── index.html            → private project-management tool — see §2f.
│   │                            Never linked from index.html or any public file.
│   ├── admin.css              → admin-only layout (reuses variables.css,
│   │                            buttons.css, work.css from ../css/)
│   └── admin.js                → storage abstraction + add/edit/delete logic
├── config/
...
- Aspect ratio is intentionally not fixed — projects span 16:9, 9:16
  (vertical short-form), or other crops. The `<video>` sizes itself
...
and Instagram are *not* re-entered anywhere in `config.contact` —
`js/contact.js` reads the existing `config.social` array directly (the
same links the footer already renders) and only adds one new thing on
top: `config.contact.email`. If you add a new entry to `config.social`,
it automatically appears as a Contact method too, with no
Contact-specific config needed.

**Icon set:** `js/contact.js` keeps its own small local copy of the
`discord`/`instagram` glyphs (plus a new `email` glyph) rather than
importing `js/content-loader.js`'s icon map — this matches the precedent
`js/work.js` already set of each section file owning a small local icon
set instead of sharing one across files.
...
## 2f. Private Management System

`admin/index.html` + `admin/admin.css` + `admin/admin.js`. A separate,
self-contained page for adding/editing/deleting project entries and
previewing a logo replacement, without touching `config/site.config.js`
by hand for every small edit.

**This is not a secure admin panel — it's an unlisted drafting tool.**
GitHub Pages is static hosting: nothing client-side can write back to the
repository, so there is no real "save to the live site" button, no
embedded credentials, and no password gate that would only create a false
sense of security. Its only protection is that its URL isn't linked from
anywhere on the public site — no nav entry, no footer link, no reference
in `index.html` at all. **Never add one.** If a real backend is ever added
(see below), this is the first assumption to revisit.

**Storage abstraction:** every read/write goes through a single `Storage`
object in `admin.js` (`Storage.list()` / `Storage.save()` /
`Storage.remove()`), currently backed by `localStorage` — this browser,
this device only. The rest of the file never touches `localStorage`
directly. This is the seam a future real backend/CMS integration would
replace: swap the three functions in `Storage`, and the form, list,
preview, and export logic all keep working unchanged.

**How a project actually gets published**, since there is no live write
path: fill out the form (title, category from `config.categories`,
thumbnail/video *paths* — file pickers only generate a local preview, the
real files still have to be placed in `assets/thumbnails/` /
`assets/videos/` by hand), save it to the local draft list, then use the
**Export** section to copy the generated `projects: [...]` array text and
paste it over the real array in `config/site.config.js` yourself. This
mirrors — and is fully compatible with — the same manual-edit workflow
documented in README.md for editing config directly; the admin page is a
nicer way to *generate* that edit, not a replacement for making it.

**Known limitation:** the local draft list is seeded from
`config.projects` the first time it's opened in a given browser, then
lives independently in `localStorage`. If you edit `config.projects`
directly later (bypassing the admin page), the admin page's draft list
won't automatically pick up that change — clear this page's local storage
for this origin if you need the two to match again.

**Logo management** is preview-only for the same reason: a file picker
shows what a replacement mark would look like, but the instructions next
to it are the actual mechanism — replace `assets/images/logo/logo.svg`
(or update `config.logo.path`) by hand.

**Visual language:** `admin/index.html` links the public site's own
`variables.css`, `buttons.css`, and `work.css` (for the live project-card
preview, so it's pixel-identical to the real thing) rather than
reimplementing any of it — this page should read as the same product, not
a bolted-on developer dashboard.
